...
def split_statements(self):
    ...
    # Standalone keywords that form full statements without semicolons
    standalone = {
        "ELSE",
        "END IF",
        "END WHILST",
    ...
    while i < len(self.text):
        ...
        if c == '\n':
            ...
            if not in_string:
                ...
                # If the line is a standalone keyword, emit it as a statement
                if possible in standalone:
                    statements.append((possible, start_line))
                    current = []
                    start_line = line_no
                else:
                    # Otherwise treat newline as space
                    current.append(' ')
...
class ExpressionParser:
    ...
    def parse_comparison(self):
        ...
        if self.text[self.pos :].startswith("IS"):
            ...
            # default to equality when the next text is an expression
